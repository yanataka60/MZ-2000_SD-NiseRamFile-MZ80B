...
//			gpio_put(32,true);
		...
		// check Memory read
			...
			if (graphic_write_enable){
				address = (bus >> 8) & 0xffff;
			    uint8_t* gvram = NULL;
			    // plane選択
			    if (graphic_write_plane == 0x01) {
			        gvram = gvram1;
			    } else if (graphic_write_plane == 0x02) {
			        gvram = gvram2;
			    }

			    // アクセス範囲判定
			    bool hit =
			        (vram_adrs && (address >= 0x6000 && address < 0x8000)) ||
			        (!vram_adrs && (address >= 0xE000));
			    if (gvram && hit) {
			        uint8_t data = gvram[address & 0x1FFF];
